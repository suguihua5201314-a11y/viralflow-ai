...
test("Test G: product workspace keeps the Director API and video-generation boundary intact",async()=>{const [source,route,panel,action]=await Promise.all([ui(),readFile(new URL("../app/api/director/route.ts",import.meta.url),"utf8"),readFile(new URL("../app/director-shot-image.tsx",import.meta.url),"utf8"),readFile(new URL("../app/image-generation-action.ts",import.meta.url),"utf8")]);for(const marker of ["DIRECTOR BRIEF","AI 副导演","Image Prompt","Video Prompt","本阶段不调用视频模型"])assert.ok(source.includes(marker),marker);assert.match(panel,/generateAndSaveImage/);assert.match(action,/\/api\/images\/generate/);assert.doesNotMatch(source+panel+action,/\/api\/(?:director-image|video-generate|generate-video)/);assert.match(route,/callProvider/);});
