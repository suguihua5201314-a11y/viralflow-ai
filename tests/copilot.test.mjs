...
test("E/F/G source preserves preview accept undo versions and existing studio capabilities",async()=>{
  ...
  for(const marker of ["修改前","修改后","放弃，不修改","接受并替换","undoLastRewrite","saveVersion","onGenerateRace","adoptRace","compareOpen","checkCompliance","scoreScript"])assert.ok(source.includes(marker),`missing ${marker}`);
  assert.match(source,/V\$\{versions\.length\s*\+\s*1\}/);
