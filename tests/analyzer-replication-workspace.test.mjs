...
test("M project mode is shared without changing schemas",()=>{assert.match(sidebar,/active==="breakdown"\|\|active==="replicate"/);assert.match(page,/vf-analyzer-mode/);assert.match(page,/vf-replication-mode/);assert.doesNotMatch(restore,/PROJECT_MEMORY_VERSION|ProjectAssetBundle\s*=/)});
