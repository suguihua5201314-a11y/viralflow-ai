...
test("M global tools keep Analyzer and Replication routes without changing schemas",()=>{assert.match(sidebar,/id: "breakdown", label: "爆款洞察"/);assert.match(sidebar,/id: "replicate", label: "创意复刻"/);assert.match(page,/active==="breakdown"\|\|active==="replicate"/);assert.doesNotMatch(restore,/PROJECT_MEMORY_VERSION|ProjectAssetBundle\s*=/)});
