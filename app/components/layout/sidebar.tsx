...
export default function Sidebar({ active, onNavigate, onHistory, counts, teamConnected, onTeamToggle, project }: SidebarProps) {
  ...
  return <aside className="sidebar vf-sidebar">
    <button className="brand vf-brand" type="button" onClick={() => onNavigate("dashboard")} aria-label="返回工作台首页"><span className="brand-mark vf-brand-mark">V</span><div><strong>ViralFlow AI</strong><small>AI短视频创作工作台</small></div></button>
    ...
    {projectMode?<div className="vf-director-sidebar-foot"><span>项目大脑</span><div><i/><b>项目素材已同步</b></div><small>{active==="director"?"分镜 · 图片 · 智能分析":active==="create"?"脚本 · 版本 · 合规":active==="breakdown"?"开场 · 结构 · 爆款洞察":"映射 · 创意 · 事实校验"}</small></div>:<div className={`sidebar-note team-note vf-team-card ${teamConnected ? "connected" : ""}`}><span>{teamConnected ? "● 团队云端已连接" : "团队云端空间"}</span><p>{teamConnected ? "产品资料、素材和历史脚本将同步给团队。" : "连接后，多台电脑可共享产品资料和历史脚本。"}</p><button onClick={onTeamToggle}>{teamConnected ? "断开本机" : "连接团队空间"}</button></div>}
  </aside>;
}
