...
export default function DirectorShotImage({ shot, input, visualStyle, projectId, onNavigate }: { shot: WorkspaceShot; input: DirectorRequest; visualStyle: string; projectId: string | null; onNavigate?: () => void }) {
  ...
  return <section className="shot-image-panel" aria-label={`镜头 ${shot.order} 图片生成`}>
    ...
    {asset ? <div className="shot-image-result"><button type="button" className="shot-image-preview" onClick={() => setShowLarge(true)} aria-label="查看镜头大图"><Image src={asset.imageUrl} alt={asset.prompt} fill sizes="(max-width: 980px) 100vw, 600px" unoptimized /></button><div><span>IMAGE PROMPT</span><p>{asset.prompt}</p><small>{asset.model} · {new Date(asset.createdAt).toLocaleString("zh-CN")}</small><nav><button type="button" onClick={() => setShowLarge(true)}>查看大图</button><button type="button" disabled={status === "loading"} onClick={() => void generateImage()}>重新生成</button><button type="button" onClick={saveAsset}>保存到图片资产</button><button type="button" onClick={openImageStudio}>前往 AI 图片创作工作台</button></nav></div></div> : <div className="shot-image-empty"><p>{spec?.prompt || "当前 Director 尚未关联项目，无法保存镜头图片。"}</p><button type="button" disabled={!projectId} onClick={openImageStudio}>前往 AI 图片创作工作台继续编辑</button></div>}
