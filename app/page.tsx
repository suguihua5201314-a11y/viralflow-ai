...
export default function Home() {
  ...
  const [form, setForm] = useState<ScriptForm>(initialScriptForm);
  ...
  const creativeDirectionInputs=useRef<Record<string,{input:CreativeBrainInput;recentScriptRevisionIds:string[]}>>({});
  ...
  function restoreProjectProjection(memory:ProjectMemory,projectId:string|null){
    ...
    const workspace=resolveProjectWorkspace(memory,projectId);
    const restored=restoreProjectScriptState(project,workspace);
    const demo=DATA_MODE==="demo"&&project?.id===demoProjects[0]?.key;
    const nextForm=initialProjectForm(project,workspace);
  ...
  useEffect(()=>{
    ...
    const hydrate=(resolution:ProjectMemoryResolution)=>{
      ...
      restoreProjectProjection(memory,snapshot.currentProjectId);
  ...
  useEffect(()=>{
    ...
    restoreProjectProjection(projectMemory,projectMemory.workspace.currentProjectId);
  ...
  function persistDirectorWorkspace(projectId:string,request:DirectorRequest,value:unknown){
    ...
    mutateMemory(current=>{
      const activate=sameDirectorRequestIdentity(capturedIdentity,activeDirectorIdentity.current);
      const projects=current.projects.map(project=>{if(project.id!==projectId)return project;const saved=saveDirectorWorkspace(project,request,value,activate);return saved===project?project:touchProject(saved,activate?{stage:"导演",progress:72}:{});});
      return{...current,projects,workspace:activate?{...current.workspace,activeView:"director"}:current.workspace,updatedAt:new Date().toISOString()};
    });
  }
  function saveWorkspaceSnapshot(patch:Partial<Pick<ProjectWorkspaceSnapshot,"activeView"|"currentProjectId"|"form"|"currentScript"|"raceResults"|"referenceScript"|"directorContext">>&Partial<ProjectWorkspaceState>){
    mutateMemory(current=>{
      const currentProjectId=patch.currentProjectId===undefined?current.workspace.currentProjectId:patch.currentProjectId;
      let next:ProjectMemory={...current,workspace:{...current.workspace,...(patch.activeView===undefined?{}:{activeView:patch.activeView}),...(patch.currentProjectId===undefined?{}:{currentProjectId:patch.currentProjectId})}};
      if(!currentProjectId)return next;
      const scopedPatch:Partial<ProjectWorkspaceState>={};
      if("form" in patch)scopedPatch.form=patch.form;
      if("referenceScript" in patch)scopedPatch.referenceScript=patch.referenceScript;
      if("directorContext" in patch)scopedPatch.directorContext=patch.directorContext;
      if("selectedScriptRevisionId" in patch)scopedPatch.selectedScriptRevisionId=patch.selectedScriptRevisionId;
  ...
  function duplicateProject(id:string){const copyId=`project-${Date.now()}`;mutateMemory(current=>{const source=current.projects.find(project=>project.id===id);if(!source)return current;const now=new Date().toISOString();const copy:PersistentProject={...source,id:copyId,name:`${source.name} 副本`,createdAt:now,updatedAt:now,assets:{...source.assets,scriptVersions:[...source.assets.scriptVersions]}};return cloneProjectWorkspace({...current,projects:[copy,...current.projects],workspace:{...current.workspace,currentProjectId:copy.id,activeView:"projects"}},id,copyId);});setSelectedProjectKey(copyId);}
  function deleteProject(id:string){mutateMemory(current=>{const projects=current.projects.filter(project=>project.id!==id);const nextId=current.workspace.currentProjectId===id?projects[0]?.id||null:current.workspace.currentProjectId;return removeProjectWorkspace({...current,projects,workspace:{...current.workspace,currentProjectId:nextId}},id);});setSelectedProjectKey(null);}
  ...
  function adoptCurrentScript(script:Script,context:AdoptedDirectorContext,sourceType:DirectorSourceType=directorSourceType){const adopted=ensureScriptRevision(script);setResult(adopted);setAdoptedDirectorContext(context);setDirectorSourceType(sourceType);updateProjectMemory({scriptVersions:[...((projectMemory.projects.find(x=>x.id===projectMemory.workspace.currentProjectId)?.assets.scriptVersions||[]).filter(item=>scriptRevisionIdentity(item as Script)!==adopted.revisionId)),adopted]},{stage:"脚本",progress:55});saveWorkspaceSnapshot({currentScript:adopted,directorContext:{...context,sourceType}});return adopted;}
  ...
  async function generateCreativeDirections(controls:GenerationControls) {
    ...
    const brainInput:CreativeBrainInput={
      ...
      preferences:{creationMode:controls.creationMode,hookStrategy:controls.hookStrategy,framework:form.framework,creativity:controls.creativity},
      sourceContext:referenceScript.trim()?{kind:"reference-script",sourceId:`project-reference:${projectId}`,referenceText:referenceScript}:undefined,
      recentCreativeHistory:recent,
    };
    try{
  ...
  function selectCreativeDirection(opportunity:CanonicalCreativeOpportunity){
    const projectId=projectMemory.workspace.currentProjectId;
    if(!projectId)return;
    const capturedProject=projectMemory.projects.find(item=>item.id===projectId);
    const capturedInput=creativeDirectionInputs.current[projectId];
    if(!capturedProject||!capturedInput){
      setCreativeDirectionSessions(sessions=>({...sessions,[projectId]:{...(sessions[projectId]||emptyCreativeDirectionSession()),selectingOpportunityId:undefined,error:capturedProject?"创意方向上下文已失效，请重新生成。":"目标项目已不存在，未保存创意方向。"}}));
      return;
    }
    setCreativeDirectionSessions(current=>markCreativeDirectionSelection(current,projectId,opportunity.id,false));
    setProjectMemory(current=>{
      return selectCreativeOpportunity(current,memoryWriterId.current||getOrCreateProjectMemoryWriterId(),{
        projectId,
        opportunity,
        productContext:capturedInput.input.productContext,
        market:capturedInput.input.market,
        language:capturedInput.input.language,
        platform:capturedInput.input.platform,
        preferences:capturedInput.input.preferences,
        sourceContext:capturedInput.input.sourceContext,
        recentScriptRevisionIds:capturedInput.recentScriptRevisionIds,
      }).memory;
    });
    setCreativeDirectionSessions(sessions=>markCreativeDirectionSelection(sessions,projectId,opportunity.id,true));
  }
  ...
  const persistentDirectorWorkspace=resolveDirectorWorkspace(currentDirectorProject,directorInput);
  ...
  },[currentDirectorProject?.id,expectedDirectorContextId,storedDirectorContextId]);
  ...
  const openRecent=(key:string,destination:"script"|"director")=>{
    ...
    const sourceType=destination==="director"?(key==="current"?directorSourceType:"history"):directorSourceType;
    adoptCurrentScript(item,{duration,offer:key==="current"?adoptedDirectorContext.offer:""},sourceType);
