...
export default function Home() {
  ...
  async function generate(controls:GenerationControls) {
    ...
    try {
      ...
      const nextHistory = [data.script, ...history].slice(0, 100); adoptCurrentScript(data.script,{duration:Number(form.duration)||30,offer:form.offer}); setHistory(nextHistory); localStorage.setItem("viralcraft-history", JSON.stringify(nextHistory)); void syncTeam(currentTeamPayload({ history: nextHistory }));
  ...
  async function generateRace(controls:GenerationControls) {
    ...
    try {
      ...
      setRaceResults(scripts); adoptCurrentScript(scripts[0],{duration:Number(form.duration)||30,offer:form.offer}); setHistory(nextHistory); localStorage.setItem("viralcraft-history", JSON.stringify(nextHistory)); void syncTeam(currentTeamPayload({ history: nextHistory }));
  ...
  >
      ...
      {active === "reviews" && <header className="review-header"><div><p className="eyebrow">发布与复盘</p><h1>发布与数据复盘</h1><p>把每条视频的数据变成下一条内容的具体优化动作。</p></div><div className="status"><i /> {reviewRecords.length}条记录</div></header>}
      ...
      {active === "products" && <header className="product-center-header"><div><p className="eyebrow">产品知识库</p><h1>产品资料中心</h1><p>团队统一产品口径，减少卖点写错、参数遗漏和违规表达。</p></div><div className="status"><i /> {productProfiles.length}个产品</div></header>}
