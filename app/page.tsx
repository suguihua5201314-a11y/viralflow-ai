...
type TeamPayload = { hooks?: HookItem[]; points?: SellingPointItem[]; products?: ProductProfile[]; reviews?: ReviewRecord[]; history?: Script[] };
...
export default function Home() {
  ...
  function currentTeamPayload(overrides: TeamPayload = {}): TeamPayload { return { hooks: hookLibrary, points: pointLibrary, products: productProfiles, reviews: reviewRecords, history, ...overrides }; }
  async function connectTeam() {
    ...
    try {
      ...
      setHookLibrary(hooks); setPointLibrary(points); setProductProfiles(products); setReviewRecords(reviews); setHistory(scripts); setTeamConnected(true); setShowTeamLogin(false);
      localStorage.setItem("susu-hook-library", JSON.stringify(hooks)); localStorage.setItem("susu-point-library", JSON.stringify(points)); localStorage.setItem("susu-product-center", JSON.stringify(products)); localStorage.setItem("susu-publish-reviews", JSON.stringify(reviews)); localStorage.setItem("viralcraft-history", JSON.stringify(scripts));
      if (!data.payload) await fetch(teamApi, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ password: teamPassword, action: "save", payload: { hooks, points, products, reviews, history: scripts } }) });
