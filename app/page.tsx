...
export default function Home() {
  ...
  function replicateViralCase(item:ViralCase){setReplicationCase(item);setReferenceScript(viralCaseReference(item));setActive("replicate");}
  function adoptReplication(script:StructuredScript,reference:string,setup:ReplicationSetup,next:"create"|"director"="create"){const generated=script as Script;const adopted={...script,id:Date.now(),product:generated.product||form.product,language:generated.language||setup.language,country:generated.country||setup.market,style:script.style||form.style,alternateHooks:generated.alternateHooks||[],scenes:script.scenes||[],sourceCaseReference:reference} as Script;setForm(prev=>({...prev,product:adopted.product,language:adopted.language,country:adopted.country,duration:setup.duration}));setReferenceScript(reference);setResult(adopted);setRaceResults([]);setDirectorSourceType("viral-replication");setActive(next);}
  ...
  const directorInput:DirectorRequest|null=result?{script:result as StructuredScript,context:{product:result.product||form.product,sellingPoints:form.sellingPoints,audience:form.audience,market:result.country||form.country,language:result.language||form.language,platform:"TikTok",targetDuration:Number(form.duration)||30,creativeMode:result.style||form.style,hookStrategy:result.hookType||"好奇",framework:result.framework||form.framework,creativeAngle:result.creativeAngle||"",offer:form.offer,productKnowledge:directorProduct,referenceScript:referenceScript||undefined,sourceType:directorSourceType,...(directorSourceType==="viral-replication"?{replication:{sourceMechanism:referenceScript.slice(0,700),replicationStrategy:referenceScript,proofMapping:result.proof||"",sellingPointMapping:result.sellingPoints||""}}:{})},settings:{provider:"deepseek"}}:null;
  ...
  >
      ...
      {active === "breakdown" ? <ViralAnalyzer cases={viralCases} product={form.product} market={form.country} language={form.language} onSave={item=>saveViralCases([item,...viralCases.filter(x=>x.id!==item.id)])} onReplicate={replicateViralCase}/> : active === "replicate" ? <ViralReplication cases={viralCases} initialCase={replicationCase} products={productProfiles} form={{...form,platform:"TikTok"}} onAdopt={(script,reference,setup)=>adoptReplication(script,reference,setup,"create")} onDirector={(script,reference,setup)=>adoptReplication(script,reference,setup,"director")} onAnalyze={()=>setActive("breakdown")}/> : active === "create" ? <ScriptStudio
        mode={active}
        form={form}
