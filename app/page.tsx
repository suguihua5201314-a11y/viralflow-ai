...
export default function Home() {
  ...
  function replicateViralCase(item:ViralCase){setReferenceScript(viralCaseReference(item));if(item.source.product)setForm(prev=>({...prev,product:item.source.product||prev.product,country:item.source.market||prev.country,language:item.source.language||prev.language}));setActive("replicate");}
  ...
  >
      ...
      {active === "breakdown" ? <ViralAnalyzer cases={viralCases} product={form.product} market={form.country} language={form.language} onSave={item=>saveViralCases([item,...viralCases.filter(x=>x.id!==item.id)])} onReplicate={replicateViralCase}/> : (active === "create" || active === "replicate") ? <ScriptStudio
        mode={active}
        form={form}
