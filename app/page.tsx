...
export default function Home() {
  ...
  const directorInput:DirectorRequest|null=result?{script:result as StructuredScript,context:{product:result.product||form.product,sellingPoints:result.sellingPoints||directorProduct?.sellingPoints||form.sellingPoints,audience:form.audience,market:result.country||form.country,language:result.language||form.language,platform:"TikTok",targetDuration:Number(form.duration)||30,creativeMode:result.style||form.style,hookStrategy:result.hookType||"好奇",framework:result.framework||form.framework,creativeAngle:result.creativeAngle||"",offer:form.offer,productKnowledge:directorProduct,sourceType:directorSourceType,...(directorSourceType==="viral-replication"?{replication:{sourceMechanism:result.creativeAngle||result.hookType||"",replicationStrategy:`${result.hookType||""} · ${result.framework||""}`,proofMapping:result.proof||"",sellingPointMapping:result.sellingPoints||""}}:{})},settings:{provider:"deepseek"}}:null;
