...
type TeamPayload = { hooks?: HookItem[]; points?: SellingPointItem[]; products?: ProductProfile[]; history?: Script[] };
...
export default function Home() {
  const [active, setActive] = useState<"create" | "breakdown" | "replicate" | "checker" | "library" | "products" | "history" | "monitor">("create");
  ...
  function currentTeamPayload(overrides: TeamPayload = {}): TeamPayload { return { hooks: hookLibrary, points: pointLibrary, products: productProfiles, history, ...overrides }; }
  async function connectTeam() {
    ...
    try {
      ...
      setHookLibrary(hooks); setPointLibrary(points); setProductProfiles(products); setHistory(scripts); setTeamConnected(true); setShowTeamLogin(false);
      localStorage.setItem("susu-hook-library", JSON.stringify(hooks)); localStorage.setItem("susu-point-library", JSON.stringify(points)); localStorage.setItem("susu-product-center", JSON.stringify(products)); localStorage.setItem("viralcraft-history", JSON.stringify(scripts));
      if (!data.payload) await fetch(teamApi, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ password: teamPassword, action: "save", payload: { hooks, points, products, history: scripts } }) });
