...
export async function POST(request: Request) {
  try {
    ...
    if (!expected || body.password !== expected) {
      return Response.json({ error: "团队密码不正确" }, { status: 401, headers });
    }
    // @ts-expect-error Cloudflare injects this module in the hosted runtime.
    const { env } = await import("cloudflare:workers");
    if (!env.DB) return Response.json({ error: "云端数据库未连接" }, { status: 503, headers });
    ...
    return Response.json({ payload: row ? JSON.parse(row.payload) : null, updatedAt: row?.updated_at ?? null }, { headers });
  } catch {
    return Response.json({ error: "团队数据服务暂时不可用" }, { status: 500, headers });
  }
}
