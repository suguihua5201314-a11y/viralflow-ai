...
export default function ScriptStudio(props:Props) {
  ...
  return <section className="script-studio">
    ...
    <div className="studio-grid">
      ...
      <aside className="studio-column studio-settings"><div className="studio-card-head"><div><span>AI CONTROLS</span><h2>AI 创作设置</h2></div><small className={providerReady?"provider-on":"provider-off"}>{providerReady?"在线":"本地"}</small></div><section className="setting-group model-selector"><header><b>AI Model</b><span>{selectedProviderStatus?.configured?"已连接":"未配置"}</span></header><select aria-label="AI Model" value={props.selectedProvider} onChange={e=>props.onProviderChange(e.target.value as ProviderId)}>{(["deepseek","doubao","openai"] as ProviderId[]).map(id=>{const item=props.providerStatuses[id];return <option value={id} key={id}>{item.label} · {item.configured?"已连接":"未配置"}</option>})}</select>{!providerReady&&<p className="provider-warning">缺少：{selectedProviderStatus?.missingFields.join("、")}</p>}</section><section className="setting-group"><header><b>创作模式</b><span>已接入</span></header><div className="choice-grid">{creationModes.map(mode=><button className={creationMode===mode?"selected":""} onClick={()=>setCreationMode(mode)} key={mode}>{mode}</button>)}</div></section><section className="setting-group"><header><b>Hook Strategy</b><span>已接入</span></header><div className="choice-grid three">{hookStrategies.map(strategy=><button className={hookStrategy===strategy?"selected":""} onClick={()=>setHookStrategy(strategy)} key={strategy}>{strategy}</button>)}</div></section><section className="setting-group"><header><b>Framework</b><span>已接入</span></header><select value={props.form.framework} onChange={e=>props.onUpdate("framework",e.target.value)}><option>智能随机</option>{compactFrameworks.map(name=><option key={name}>{name}</option>)}{props.frameworks.filter(item=>!compactFrameworks.includes(item.name)).map(item=><option key={item.name}>{item.name}</option>)}</select></section><section className="setting-group"><header><b>Creativity</b><span>已接入</span></header><div className="studio-segments">{["稳定","平衡","激进"].map(value=><button className={creativity===value?"selected":""} onClick={()=>setCreativity(value)} key={value}>{value}</button>)}</div></section><section className="setting-group"><header><b>Output</b><span>已接入</span></header><div className="studio-pair"><label>时长<select value={props.form.duration} onChange={e=>props.onUpdate("duration",e.target.value)}><option value="30">30s</option><option value="45">45s</option><option value="60">60s</option></select></label><label>语言<select value={props.form.language} onChange={e=>props.onUpdate("language",e.target.value)}>{props.languages.map(x=><option key={x}>{x}</option>)}</select></label></div><label>生成数量<div className="studio-segments"><button className={outputCount===1?"selected":""} onClick={()=>setOutputCount(1)}>1 条</button><button className={outputCount===5?"selected":""} onClick={()=>setOutputCount(5)}>5 条赛马</button></div></label></section><div className="provider-card"><span>PROVIDER STATUS</span><strong>{props.lastProviderRun ? props.lastProviderRun.providerRequested+" → "+props.lastProviderRun.providerUsed : (selectedProviderStatus?.label||"Provider")+" "+(providerReady?"已连接":"未配置")}</strong><p>{props.lastProviderRun ? "AI="+(props.lastProviderRun.aiGenerated?"是":"否")+" · Fallback="+(props.lastProviderRun.fallbackUsed?"是":"否")+" · "+(props.lastProviderRun.providerErrorType||"无错误")+" · "+(props.lastProviderRun.responseTimeMs??"—")+"ms" : "切换只影响下一次生成，不会重置 Brief 或创作控制项。"}</p></div><div className="generation-summary"><span>GENERATION SUMMARY</span><p>{generationSummary}</p></div>{props.error&&<p className="studio-error">{props.error}</p>}<button className="studio-generate" disabled={!props.inputReady||props.loading||props.raceLoading||!providerReady} onClick={()=>{setOutputCount(1);props.onGenerate(controls(1))}}>{props.loading?"正在生成…":"✦ 生成单条脚本"}</button><button className="studio-race" disabled={!props.inputReady||props.loading||props.raceLoading||!providerReady} onClick={()=>{setOutputCount(5);props.onGenerateRace(controls(5))}}>{props.raceLoading?"赛马生成中…":"生成 5 条赛马稿"}</button></aside>
    </div>
