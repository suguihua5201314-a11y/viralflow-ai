...
export default function ScriptStudio(props:Props) {
  ...
  return <section className="script-studio">
    <header className="studio-titlebar"><div><span className="studio-kicker">PROJECT CREATIVE OS</span><h1>AI Script <em>/</em> Script Workspace</h1><p>用 Brief 驱动创意赛马，在同一个编辑器完成改稿、合规与下一步制作。</p></div><div className="studio-title-actions"><button onClick={()=>props.onNavigate("history")}>历史脚本 <b>{props.historyCount}</b></button><span className={providerReady?"provider-chip online":"provider-chip"}><i/>{providerReady?"Provider 在线":"本地模式"}</span></div></header>
    ...
    <div className="studio-grid">
      ...
      <main className="studio-center">
        <section className="script-version-rail"><header><div><span>SCRIPT VARIATIONS</span><b>{props.raceResults.length?`${props.raceResults.length} 个赛马版本`:"Version Rail"}</b></div><nav><button disabled={compare.length!==2} onClick={()=>setCompareOpen(true)}>对比 {compare.length}/2</button><button disabled={!props.inputReady||props.loading||props.raceLoading||!providerReady} onClick={()=>{set生成数量Count(5);props.onGenerateRace(controls(5))}}>{props.raceLoading?"生成中…":"＋ 新赛马"}</button></nav></header><div>{props.raceResults.length?props.raceResults.map((item,index)=>{const itemScore=props.scoreScript(item);const isCurrent=Boolean(script&&variantIdentity(script)===variantIdentity(item));return <article key={`${item.title}-${index}`} className={`${isCurrent?"active":""} ${compare.includes(index)?"comparing":""}`}><button className="version-select" onClick={()=>adoptRace(item)}><small>V{index+1}{isCurrent&&<i>主版本</i>}</small><b>{item.style||item.hookType||letters[index]}</b><span>{item.hook}</span><em><small>SCORE</small>{itemScore.total}</em></button><button className="version-compare" aria-label={`选择 V${index+1} 对比`} onClick={()=>toggleCompare(index)}>{compare.includes(index)?"✓":"◇"}</button></article>}):letters.map((letter,index)=><article className="empty" key={letter}><button className="version-select" disabled><small>V{index+1}</small><b>{["KOC / UGC","Strong Conflict","Product Demo","Curiosity","Storytelling"][index]}</b><span>等待生成</span></button></article>)}</div></section>
