  ...
  return <section className="script-studio">
    ...
    <div className="studio-grid">
      ...
      <main className="studio-center">
        <section className="script-version-rail"><header><div><span>SCRIPT VARIATIONS</span><b>{props.raceResults.length?`${Math.min(props.raceResults.length,5)} 个赛马版本`:"Version Rail · 尚无版本"}</b></div><nav><button disabled={compare.length!==2} onClick={()=>setCompareOpen(true)}>对比 {compare.length}/2</button><button disabled={!props.inputReady||props.loading||props.raceLoading||!providerReady} onClick={()=>{set生成数量Count(5);props.onGenerateRace(controls(5))}}>{props.raceLoading?"生成中…":"＋ 新赛马"}</button></nav></header><div>{versionSlots.map(({item,index,direction})=>{if(item){const itemScore=props.scoreScript(item);const isCurrent=Boolean(script&&variantIdentity(script)===variantIdentity(item));return <article key={`${item.title}-${index}`} className={`${isCurrent?"active":""} ${compare.includes(index)?"comparing":""}`}><button className="version-select" onClick={()=>adoptRace(item)}><small>V{index+1}{isCurrent&&<i>当前版本</i>}</small><b>{item.style||item.hookType||direction}</b><span>{item.hook}</span><em><small>SCORE</small>{itemScore.total}</em></button><button className="version-compare" aria-label={`选择 V${index+1} 对比`} onClick={()=>toggleCompare(index)}>{compare.includes(index)?"✓":"◇"}</button></article>}return <article className="version-empty" key={`empty-v${index+1}`}><div className="version-select"><small>V{index+1}</small><b>{direction}</b><span>待生成</span><button className="version-slot-generate" disabled={!props.inputReady||props.loading||props.raceLoading||!providerReady} onClick={()=>{set生成数量Count(5);props.onGenerateRace(controls(5))}}>{props.raceLoading?"生成中…":"生成此版本"}</button></div></article>})}</div></section>
