...
function buildBlocks(script:StudioScript, sellingPoints:string):EditorBlock[] {
  ...
  return [
    ...
    { key:"conflict", label:"冲突 / 问题", type:"观看理由", duration:"3–8s", text:scenes[1]?.line || "当前脚本未单独标注冲突" },
    { key:"product", label:"产品出现", type:"产品入场", duration:"8–15s", text:scenes[2]?.line || scenes[0]?.line || script.product },
    { key:"proof", label:"演示 / 证明", type:"动作证明", duration:"15–21s", text:scenes.slice(2,-2).map(x=>x.line).filter(Boolean).join(" ") || "当前脚本未单独标注演示" },
    { key:"points", label:"卖点递进", type:"价值建立", duration:"21–26s", text:scenes.at(-2)?.line || sellingPoints },
    { key:"cta", label:"CTA", type:"行动引导", duration:"26–30s", text:scenes.at(-1)?.line || "当前脚本未单独标注 CTA" },
  ];
}
...
export default function ScriptStudio(props:Props) {
  ...
  return <section className="script-studio">
    ...
    <div className="studio-grid">
      ...
      <aside className="studio-column studio-settings"><div className="studio-card-head"><div><span>AI CONTROLS</span><h2>AI 创作设置</h2></div><small className={props.aiConnected?"provider-on":"provider-off"}>{props.aiConnected?"在线":"本地"}</small></div><section className="setting-group"><header><b>创作模式</b><em className="pending-badge">待接入</em></header><div className="choice-grid">{creationModes.map(mode=><button className={creationMode===mode?"selected":""} onClick={()=>setCreationMode(mode)} key={mode}>{mode}</button>)}</div></section><section className="setting-group"><header><b>Hook Strategy</b><em className="pending-badge">待接入</em></header><div className="choice-grid three">{hookStrategies.map(strategy=><button className={hookStrategy===strategy?"selected":""} onClick={()=>setHookStrategy(strategy)} key={strategy}>{strategy}</button>)}</div></section><section className="setting-group"><header><b>Framework</b><span>已接入</span></header><select value={props.form.framework} onChange={e=>props.onUpdate("framework",e.target.value)}><option>智能随机</option>{compactFrameworks.map(name=><option key={name}>{name}</option>)}{props.frameworks.filter(item=>!compactFrameworks.includes(item.name)).map(item=><option key={item.name}>{item.name}</option>)}</select></section><section className="setting-group"><header><b>Creativity</b><em className="pending-badge">待接入</em></header><div className="studio-segments">{["稳定","平衡","激进"].map(value=><button className={creativity===value?"selected":""} onClick={()=>setCreativity(value)} key={value}>{value}</button>)}</div></section><section className="setting-group"><header><b>Output</b><span>已接入</span></header><div className="studio-pair"><label>时长<select value={props.form.duration} onChange={e=>props.onUpdate("duration",e.target.value)}><option value="30">30s</option><option value="45">45s</option><option value="60">60s</option></select></label><label>语言<select value={props.form.language} onChange={e=>props.onUpdate("language",e.target.value)}>{props.languages.map(x=><option key={x}>{x}</option>)}</select></label></div><label>生成数量<div className="studio-segments"><button className={outputCount===1?"selected":""} onClick={()=>setOutputCount(1)}>1 条</button><button className={outputCount===5?"selected":""} onClick={()=>setOutputCount(5)}>5 条赛马</button></div></label></section><div className="provider-card"><span>PROVIDER STATUS</span><strong>{props.aiConnected?"现有 AI Provider 已连接":"稳定本地脚本引擎"}</strong><p>继续使用现有 API 与 Prompt；带“待接入”的设置不会影响生成结果。</p></div><div className="generation-summary"><span>GENERATION SUMMARY</span><p>{generationSummary}</p></div>{props.error&&<p className="studio-error">{props.error}</p>}<button className="studio-generate" disabled={!props.inputReady||props.loading||props.raceLoading} onClick={()=>{setOutputCount(1);props.onGenerate()}}>{props.loading?"正在生成…":"✦ 生成单条脚本"}</button><button className="studio-race" disabled={!props.inputReady||props.loading||props.raceLoading} onClick={()=>{setOutputCount(5);props.onGenerateRace()}}>{props.raceLoading?"赛马生成中…":"生成 5 条赛马稿"}</button></aside>
    </div>
